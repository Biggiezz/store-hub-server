...
router.post("/add-review", async (req, res) => {
  try {
    ...
    if (!productId || !customerName || rating === undefined || !content) {
      return res.status(400).json({ code: 400, message: "Thiếu thông tin đánh giá" });
    }
    ...
    const totalRating = product.reviews.reduce((acc, cur) => acc + (cur.rating || 0), 0);
...
router.post("/add-product", async (req, res) => {
  try {
    const { name, price, image, category, description, rating, reviewCount, stock, colors, reviews } = req.body;
